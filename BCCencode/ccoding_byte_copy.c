...
main()
{
  ...
  for (i = 0; i < 12; i++) printf("%d ", test[i]);
  printf("\n");
  ...
  ccodedot11_encode(12, inPtr, outPtr, 0);//0 1/2; 1 3/4;2 2/3
  for (i = 0; i < 192; i++) printf("%x ", output[i]);
  printf("\n");

  int fp=fopen("bccDataOut.txt", "w");      /*建立一个文字文件只写*/
